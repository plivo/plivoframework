...
class RESTInboundSocket(InboundEventSocket):
    ...
    def modify_call(self, new_xml_url, status, call_uuid, request_uuid):
        if new_xml_url:
            ...
            '''
            bridge_uuid = self.get_var("bridge_uuid", uuid=call_uuid)
            if bridge_uuid:
                self.transfer("plivo", uuid=call_uuid)
                self.api("uuid_transfer %s -bleg 'hangup:ORIGINATOR_CANCEL' inline" % call_uuid)
            else:
                self.api("uuid_transfer %s plivo" % call_uuid)
            '''
        else:  # Hangup Call
            if call_uuid:
